...
func (s *shortenerMock) ShortenBatch(ctx context.Context, originalURLs []string) (map[string]string, error) {
	return map[string]string{s.shortenRet: s.expandRet}, s.shortenErr
}
...
func TestShorten_batch(t *testing.T) {
	tests := testTable{
		{
			name:   "successful POST",
			method: http.MethodPost,
			body:   `[{"corelation_id":"111","original_url":"https://example.com/foo"}]`,
			mock: shortenerMock{
				shortenRet: "http://test.test/abc123",
			},
			wantStatus: http.StatusCreated,
			wantCT:     "application/json",
			wantBody:   `[{"corelation_id":"111","short_url":"http://test.test/abc123"}]`,
		},
		{
			name:       "reject non‑POST",
			method:     http.MethodGet,
		...
		},
		{
			name:       "invalid URL",
			method:     http.MethodPost,
			body:       `[{"corelation_id":"111","original_url":"not-a-url"}]`,
			mock:       shortenerMock{},
			wantStatus: http.StatusBadRequest,
			wantCT:     "text/plain; charset=utf-8",
			wantBody:   "Incorrect URL\n",
		...
		},
		{
			...
			method: http.MethodPost,
			body: func() string {
				return `[{"corelation_id":"111","original_url":"https://example.com/foo` + strings.Repeat("a", 2*1024) + `"}]`
			}(),
			mock:       shortenerMock{},
			wantStatus: http.StatusBadRequest,
			wantCT:     "text/plain; charset=utf-8",
